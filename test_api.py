    ...
    def test_inspect_endpoint(self, spotify_url: str) -> Dict[str, Any]:
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/api/v1/inspect",
                params={"spotify_url": spotify_url}
            )
            return {
                "status": response.status_code,
                "data": response.json() if response.status_code == 200 else response.text
...
def main():
    ...
    inspect_result = client.test_inspect_endpoint("https://open.spotify.com/track/4iV5W9uYEdYUVa79Axb7Rh")
